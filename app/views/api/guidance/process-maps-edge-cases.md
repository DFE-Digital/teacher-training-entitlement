...
<div class="mermaid">
    ...
    provider->>+assessor:
    Note over assessor: Change funded place value of an application
...
<div class="mermaid">
    ...
    Note over provider: Send participant training portal login details
    provider->>+dfe:
...
<div class="mermaid">
    ...
    Note over provider: Submit 'Start' declaration via API against application
    provider->>+assessor:
    Note over assessor: Submit started declaration on application
    provider->>+dfe:
...
<div class="mermaid">
    ...
    Note over provider: Submit declarations at each milestone via application
    provider->>+assessor:
    Note over assessor: Submit declaration on application
    provider->>+dfe:
    Note over dfe: Declarations errors
    provider->>+dfe:
...
<div class="mermaid">
    ...
    provider->>+assessor:
    Note over assessor: Withdraw an application
    Note over provider: Notify DfE of course deferral via API
    provider->>+assessor:
    Note over assessor: Defer an application
    Note over provider: Notify DfE of participant resuming course via API
    provider->>+assessor:
    Note over assessor: Resume an application (select target cohort)
</div>
...
<div class="mermaid">
    ...
    provider->>+assessor:
    Note over assessor: Submit completed declaration on application
    provider->>+dfe:
    Note over dfe: Completed declaration error
    provider->>+assessor:
    Note over assessor: Retrieve outcomes for all participants
</div>
