...
class Admin::Finance::Statements::AdjustmentsController < AdminController
  ...
  def new
    ...
    @cancel_url = if @show_all_adjustments
                    admin_finance_statement_adjustments_path(@statement, show_all_adjustments: @show_all_adjustments)
                  else
  ...
  def create
    ...
    if @create_adjustment_form.save_adjustment
      ...
      redirect_to admin_finance_statement_adjustments_path(@statement, show_all_adjustments:)
    else
  ...
  def edit
    @cancel_url = admin_finance_statement_adjustments_path(@statement, show_all_adjustments: @show_all_adjustments)
  end
  ...
  def update
    ...
    if @update_adjustment_form.save_adjustment
      redirect_to admin_finance_statement_adjustments_path(@statement, show_all_adjustments: @show_all_adjustments)
    else
  ...
  def add_another
    ...
    if @add_another_form.invalid?
      ...
      redirect_to new_admin_finance_statement_adjustment_path(@statement, show_all_adjustments:)
    else
