...
RSpec.feature "Sad journeys", :npq, type: :feature do
  ...
  scenario "when an unintended journey leaves mandatory institution missing" do
    ...
    expect_page_to_have(path: "/reception-registration/your-employment", submit_form: true) do
      page.choose("As a teacher employed by a local authority to teach in more than one school", visible: :all)
    end

    expect_page_to_have(path: "/reception-registration/your-role", submit_form: true) do
      page.fill_in "What is your role?", with: "Teacher"
    end

    expect_page_to_have(path: "/reception-registration/your-employer", submit_form: true) do
      page.fill_in "What organisation are you employed by?", with: "Local authority"
    end

    expect_page_to_have(path: "/reception-registration/choose-your-npq", submit_form: true) do
      page.choose("Headship", visible: :all)
    end

    expect_page_to_have(path: "/reception-registration/possible-funding", submit_form: false)
    ...
    navigate_to_page(path: "/reception-registration/work-setting", submit_form: true) do
      page.choose("A school", visible: :all)
    end

    expect_page_to_have(path: "/reception-registration/choose-school", submit_form: false)

    # back to window 1 (with the store work setting now changed from window 2)
    navigate_to_page(path: "/reception-registration/choose-your-npq", submit_form: true) do
      page.choose("Headship", visible: :all)
    end

    expect_page_to_have(path: "/reception-registration/choose-school", submit_form: true)

    navigate_to_page(path: "/reception-registration/share-provider", submit_form: true) do
