...
RSpec.feature "Adjustments", :no_js, type: :feature do
  ...
  context "when not logged in" do
    ...
    scenario "adjustments page is inaccessible" do
      visit(new_admin_finance_statement_adjustment_path(statement))
      expect(page).to have_current_path(sign_in_path)
    end
  end
  ...
  context "when logged in as admin" do
    ...
    scenario "add adjustment" do
      ...
      # check cancel link
      click_on "Cancel"
      expect(page).to have_current_path(admin_finance_statement_adjustments_path(statement, show_all_adjustments: false))
      # edit for real
      click_on "Edit"
      ...
      # check cancel link
      click_on "Cancel"
      expect(page).to have_current_path(admin_finance_statement_adjustments_path(statement, show_all_adjustments: true))
      # edit for real
      within ".govuk-table__row:nth-of-type(2)" do
        click_on "Edit"
      end
    ...
    scenario "statement is marked as payable" do
      visit(admin_finance_statement_path(payable_statement))

      expect(page).to have_link "Make adjustment"
    end

    scenario "statement is marked as paid" do
      visit(admin_finance_statement_path(paid_statement))
    ...
    scenario "statement moved to paid whilst editing adjustment" do
      ...
      visit(edit_admin_finance_statement_adjustment_path(statement, adjustment))
    ...
    scenario "statement moved to paid whilst deleting adjustment" do
      ...
      visit(delete_admin_finance_statement_adjustment_path(statement, adjustment))
