...
RSpec.feature "actions log", :no_js, :versioning, type: :feature do
  ...
  before do
    ...
    application_1
    application_2
    PaperTrail.request.whodunnit = "AdminUser #{admin.id}"
  ...
  scenario "viewing an admin user's actions" do
    ...
    expect(page).to have_link("History", href: admin_applications_history_path(application_1))
    expect(page).to have_link("History", href: admin_applications_history_path(application_2))
  end
