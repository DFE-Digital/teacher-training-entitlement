...
RSpec.describe Admin::SchedulesController, :ecf_api_disabled, type: :request do
  ...
  context "when logged in as super admin" do
    ...
    describe "#show" do
      before { get admin_cohort_schedule_path(cohort, schedule) }

      it { is_expected.to have_http_status :success }
    end

    describe "#new" do
      before { get new_admin_cohort_schedule_path(cohort) }
    ...
    describe "#create with invalid params" do
      before { post admin_cohort_schedules_path(cohort), params: invalid_params }

      it { is_expected.to have_http_status :unprocessable_content }
    end

    describe "#edit" do
      before { get edit_admin_cohort_schedule_path(cohort, schedule) }

      it { is_expected.to have_http_status :success }
    end

    describe "#update" do
      before { patch admin_cohort_schedule_path(cohort, schedule), params: valid_params }

      it { is_expected.to redirect_to admin_cohort_schedule_path(cohort, schedule) }
    ...
    describe "#update with invalid params" do
      before { patch admin_cohort_schedule_path(cohort, schedule), params: invalid_params }

      it { is_expected.to have_http_status :unprocessable_content }
    end

    describe "#destroy" do
      before { delete admin_cohort_schedule_path(cohort, schedule) }
