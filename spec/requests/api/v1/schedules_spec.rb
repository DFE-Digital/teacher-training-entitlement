...
RSpec.describe "Schedule endpoints", type: :request do
  ...
  describe "GET /api/v1/schedules" do
    ...
    before { create(:cohort_provider, cohort:, lead_provider: current_lead_provider) }
    ...
    context "when authorized" do
      ...
      it "returns schedules for the lead provider" do
        ...
        expect(parsed_response["data"].first["id"]).to eq(cohort.ecf_id)
      end

      it "includes the correct attributes" do
        attrs = parsed_response["data"].first["attributes"]
        expect(attrs["course_identifier"]).to eq(course.identifier)
        expect(attrs["schedule_identifier"]).to eq(cohort.schedules.last.identifier)
      ...
      it "does not return schedules for other lead providers" do
        ...
        create(:schedule, :tte_reception_spring, cohort: other_cohort)
        # create(:course_cohort, course:, cohort: other_cohort, schedule: other_schedule, lead_provider: other_lp)
        create(:cohort_provider, cohort: other_cohort, lead_provider: other_lp)

        api_get(api_v1_schedules_path)

        expect(parsed_response["data"].pluck("id")).to contain_exactly(cohort.ecf_id)
      end
    end
