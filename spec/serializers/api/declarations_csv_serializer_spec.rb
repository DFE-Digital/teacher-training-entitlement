...
RSpec.describe API::DeclarationsCsvSerializer, type: :serializer do
  ...
  describe "#serialize" do
    ...
    let(:fields) { %i[id participant_id declaration_type course_identifier declaration_date updated_at state] }

    it { expect(rows.count).to eq(declarations.count) }
    it { expect(first_row.slice(*fields).values).to all(be_present) }
