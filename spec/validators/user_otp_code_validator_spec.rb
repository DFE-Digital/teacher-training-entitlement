...
RSpec.describe UserOTPCodeValidator do
  ...
  let(:user) { instance_double(AdminUser, otp:, otp_locked_out?: false) }
  ...
  context "when the user is locked" do
    let(:user) { instance_double(AdminUser, otp:, otp_locked_out?: true) }
  ...
  context "when the user has no usable OTP" do
    let(:user) { instance_double(AdminUser, otp: nil, otp_locked_out?: false) }
