...
RSpec.describe ParticipantNotWithdrawnValidator do
  ...
  describe "#validate" do
    ...
    context "when participant was withdrawn before declaration_date" do
      before do
        travel_to declaration_date - 10.days do
          application.state_changes.create!(event: Application::WITHDRAWN, lead_provider:)
    ...
    context "when participant was reinstated after being withdrawn" do
      ...
      before do
        application.state_changes.create!(event: Application::ACCEPTED, lead_provider:)
    ...
    context "when participant was withdrawn by another lead provider" do
      before do
        application.state_changes.create!(event: Application::WITHDRAWN, lead_provider: LeadProvider.where.not(id: lead_provider.id).first)
        application.update_columns(status: Application::WITHDRAWN)
      end
