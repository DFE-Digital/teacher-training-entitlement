...
RSpec.describe APITests::DeferApplication, type: :model do
  ...
  let(:expected_url) do
    "#{ENV.fetch("HOSTING_DOMAIN", "http://localhost:3000")}#{Rails.application.routes.url_helpers.defer_api_v1_application_path(application.ecf_id)}"
  end
  ...
  describe "#call" do
    it "sends a defer application request" do
    ...
    context "when a started application cannot be found" do
      ...
      it "raises an error" do
        expect { service.call }
          .to raise_error(RuntimeError, "[DeferApplication] Could not find a deferred application")
      end
    end
  end
end
